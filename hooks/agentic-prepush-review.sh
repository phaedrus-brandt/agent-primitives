...
#
# Tiers (chosen per push, from effective diff size x risk-path match):
...
# findings or engine errors when the toolchain is present.
...
# ------------------------------------------------------- push range from stdin
ZERO="0000000000000000000000000000000000000000"
LOCAL_SHA=""; REMOTE_SHA=""
while read -r _lref lsha _rref rsha; do
    [ -z "${lsha:-}" ] && continue
    [ "$lsha" = "$ZERO" ] && continue      # branch deletion - nothing to review
    LOCAL_SHA="$lsha"; REMOTE_SHA="${rsha:-$ZERO}"
    break                                   # review the first non-delete ref
done
[ -z "$LOCAL_SHA" ] && exit 0
...
[ -z "$BASE" ] && { echo "agentic-review: cannot determine base - skipping."; exit 0; }
[ "$BASE" = "$LOCAL_SHA" ] && exit 0        # nothing new
...
'
set -f
CHANGED=$(git diff --name-only $RANGE -- . $IGNORE 2>/dev/null)
...
[ -z "$CHANGED" ] && { echo "agentic-review: only ignored files changed - skipping."; exit 0; }
NON_PROSE=$(printf '%s\n' "$CHANGED" | grep -cvE '\.(md|txt|rst)$' || true)
if [ "$NON_PROSE" = "0" ]; then
    echo "agentic-review: prose-only diff - skipping."
    exit 0
fi
...
if [ "$STATUS" = "0" ]; then
    ...
    echo "agentic-review: clean at tier $TIER - push allowed."
    exit 0
fi
echo ""
